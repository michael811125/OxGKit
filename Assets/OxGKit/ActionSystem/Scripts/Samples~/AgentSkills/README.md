# OxGKit.ActionSystem AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.ActionSystem** — a lightweight action-sequence framework: compose delays, callbacks, sequences, parallels, and custom actions, pumped by an ActionRunner.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.ActionSystem source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.ActionSystem usage: ActionRunner update loops, default sequence/parallel/delay/delegate actions, and custom ActionBase authoring. |
...
1. Import this sample through **Package Manager > OxGKit.ActionSystem > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.ActionSystem/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.ActionSystem APIs." |
...
- API signatures were verified against OxGKit.ActionSystem v1.0.3. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

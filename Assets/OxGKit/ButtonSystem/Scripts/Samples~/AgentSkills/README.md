# OxGKit.ButtonSystem AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.ButtonSystem** — ButtonPlus, a UGUI Button extension with long-click modes and a scale press-transition, plus an InputField placeholder helper.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.ButtonSystem source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.ButtonSystem usage: ButtonPlus long-click modes, scale transition, event wiring, and the InputField placeholder remover. |
...
1. Import this sample through **Package Manager > OxGKit.ButtonSystem > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.ButtonSystem/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.ButtonSystem APIs." |
...
- API signatures were verified against OxGKit.ButtonSystem v1.0.2. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

# OxGKit.InfiniteScrollView AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.InfiniteScrollView** — pooled infinite scrolling lists on native UGUI: vertical/horizontal/grid variants, the InfiniteCell lifecycle, dynamic cell sizes, and snapping.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.InfiniteScrollView source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.InfiniteScrollView usage: scroll-view variants, InfiniteCell lifecycle, InfiniteCellData sizing, data operations, and snapping. |
...
1. Import this sample through **Package Manager > OxGKit.InfiniteScrollView > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.InfiniteScrollView/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.InfiniteScrollView APIs." |
...
- API signatures were verified against OxGKit.InfiniteScrollView v1.7.0. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

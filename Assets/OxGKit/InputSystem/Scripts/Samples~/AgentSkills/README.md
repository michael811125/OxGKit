# OxGKit.InputSystem AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.InputSystem** — an input-dispatch layer over Unity New Input System: a control-map registry plus IInputAction signal dispatchers so gameplay subscribes to game-level events.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.InputSystem source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.InputSystem usage: Inputs.CM control maps, IInputAction dispatchers, DriveUpdate loops, and binding composites. |
...
1. Import this sample through **Package Manager > OxGKit.InputSystem > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.InputSystem/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.InputSystem APIs." |
...
- API signatures were verified against OxGKit.InputSystem v1.0.3. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

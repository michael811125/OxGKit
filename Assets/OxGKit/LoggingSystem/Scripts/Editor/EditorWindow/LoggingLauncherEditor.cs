    ...
    public class LoggingLauncherEditor : UnityEditor.Editor
    {
        ...
        protected void DrawControlButtonsView(LoggersConfig loggersConfig)
        {
            ...
            if (loggersConfig != null)
            {
                ...
                if (GUILayout.Button(new GUIContent("Reload From Config", $"Load configuration only from {LoggingSettings.settings.loggerCfgName}{LoggingSettings.settings.loggerCfgExtension}."), GUILayout.MaxWidth(250f)))
                {
                    if (Application.isPlaying)
                    {
                        this._isDirty = true;
            ...
            if (loggersConfig != null)
            {
                ...
                if (GUILayout.Button(new GUIContent("Reload", $"When the {LoggingSettings.settings.loggerCfgName}{LoggingSettings.settings.loggerCfgExtension} file is modified or a new logger is added, you can use Reload."), GUILayout.MaxWidth(250f)))
                {
                    if (Application.isPlaying)
                    {
                        this._isDirty = true;

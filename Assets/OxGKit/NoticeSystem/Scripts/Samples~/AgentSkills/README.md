# OxGKit.NoticeSystem AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.NoticeSystem** — a red-dot / notification badge system: condition classes plus NoticeItem components refreshed by NoticeManager.Notify.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.NoticeSystem source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.NoticeSystem usage: NoticeCondition authoring, NoticeItem registration, and Notify-driven red-dot refresh. |
...
1. Import this sample through **Package Manager > OxGKit.NoticeSystem > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.NoticeSystem/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.NoticeSystem APIs." |
...
- API signatures were verified against OxGKit.NoticeSystem v1.0.5. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

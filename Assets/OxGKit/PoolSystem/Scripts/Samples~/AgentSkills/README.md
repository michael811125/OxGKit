# OxGKit.PoolSystem AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.PoolSystem** — NodePool, a GameObject pool component with across-frames initialization, auto-growth, and max-size capping.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.PoolSystem source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.PoolSystem usage: NodePool setup, across-frames initialization, Get/Put recycling, autoPut and maxSize behavior. |
...
1. Import this sample through **Package Manager > OxGKit.PoolSystem > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.PoolSystem/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.PoolSystem APIs." |
...
- API signatures were verified against OxGKit.PoolSystem v1.0.1. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

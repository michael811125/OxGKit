# OxGKit.SaverSystem AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.SaverSystem** — a small key/value persistence layer (PlayerPrefs/EditorPrefs implementations) with a text-content data-map feature for settings.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.SaverSystem source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.SaverSystem usage: ISaver/Saver APIs, PlayerPrefsSaver/EditorPrefsSaver, text-content data maps, and custom savers. |
...
1. Import this sample through **Package Manager > OxGKit.SaverSystem > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.SaverSystem/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.SaverSystem APIs." |

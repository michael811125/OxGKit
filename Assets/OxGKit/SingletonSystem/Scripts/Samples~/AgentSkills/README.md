# OxGKit.SingletonSystem AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.SingletonSystem** — MonoSingleton and NewSingleton bases with guarded lifecycle hooks and DontDestroyOnLoad control.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.SingletonSystem source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.SingletonSystem usage: MonoSingleton lifecycle and access patterns, NewSingleton, and DontDestroyOnLoad control. |
...
1. Import this sample through **Package Manager > OxGKit.SingletonSystem > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.SingletonSystem/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.SingletonSystem APIs." |
...
- API signatures were verified against OxGKit.SingletonSystem v1.0.1. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

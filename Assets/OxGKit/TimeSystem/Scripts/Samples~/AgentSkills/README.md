# OxGKit.TimeSystem AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.TimeSystem** — time controllers: RealTimer/DeltaTimer, RTUpdater/DTUpdater standalone update loops, IntervalSetter periodic calls, and NtpTime NTP clock sync.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.TimeSystem source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.TimeSystem usage: RealTimer/DeltaTimer, RTUpdater/DTUpdater, IntervalSetter, and NtpTime synchronization. |
...
1. Import this sample through **Package Manager > OxGKit.TimeSystem > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.TimeSystem/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.TimeSystem APIs." |
...
- API signatures were verified against OxGKit.TimeSystem v1.1.3. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

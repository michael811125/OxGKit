# OxGKit.TweenSystemFixer AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.TweenSystemFixer** — an editor tool that repairs the DOTween.Modules asmdef GUID that OxGKit.TweenSystem references.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.TweenSystemFixer source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.TweenSystemFixer usage: diagnosing the DOTween.Modules GUID breakage and repairing it with the fixer tool or manually. |
...
1. Import this sample through **Package Manager > OxGKit.TweenSystemFixer > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.TweenSystemFixer/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.TweenSystemFixer APIs." |
...
- API signatures were verified against OxGKit.TweenSystemFixer v1.0.2. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

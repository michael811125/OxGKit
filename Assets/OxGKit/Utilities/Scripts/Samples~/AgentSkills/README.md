# OxGKit.Utilities AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.Utilities** — essential helpers: Requester cached web requests, LRU/ARC caches, UI safe-area adapter, EasyAnim, TextureAnimation, DontDestroy, a main-thread dispatcher, and editor tools.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.Utilities source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.Utilities usage: Requester cached downloads, LRU/ARC caches, UISafeAreaAdapter, EasyAnim, TextureAnimation, UMT, and the editor tools. |
...
1. Import this sample through **Package Manager > OxGKit.Utilities > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.Utilities/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.Utilities APIs." |
...
- API signatures were verified against OxGKit.Utilities v1.4.8. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.

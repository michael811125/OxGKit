# OxGKit.VirtualJoystick AI Agent Skill

An Agent Skill for AI coding agents (Claude Code, OpenAI Codex CLI, Cursor, Copilot, etc.) that teaches the agent how to use **OxGKit.VirtualJoystick** — a UGUI on-screen joystick with fixed/floating modes, axis constraints, dead zone, and a Vector2 output event.

The skill follows the Agent Skills convention (a folder with a `SKILL.md` entry file: YAML frontmatter + instructions). API usage is verified against the OxGKit.VirtualJoystick source of this package version.

## What is included

| Skill | Purpose |
|---|---|
| `oxgkit-unity-skill` | OxGKit.VirtualJoystick usage: VirtualJoystick component setup, onStickInput usage, stick types/vector modes, and the UI prefab. |
...
1. Import this sample through **Package Manager > OxGKit.VirtualJoystick > Samples > AI Agent Skills**.
   The files are copied to `Assets/Samples/OxGKit.VirtualJoystick/<version>/AI Agent Skills/`.
2. Copy (or symlink) the skill folder to wherever your agent discovers skills:
   ...
   | Other agents | Reference the folder from your agent context file (`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, ...), e.g. "Read Assets/Samples/.../oxgkit-unity-skill/SKILL.md before working with OxGKit.VirtualJoystick APIs." |
...
- API signatures were verified against OxGKit.VirtualJoystick v1.0.0. Newer versions may differ — the skill instructs the agent to re-verify against the installed package source.
